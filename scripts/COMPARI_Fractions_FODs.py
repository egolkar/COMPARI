...
class DwiInfo:
    def __init__(self,args):
        ...
        if (os.path.exists(args.pickle) == True):
            with open( args.pickle , 'rb') as f:
                self.lambda_par = pickle.load(f)
                print('lambda_par',self.lambda_par )
        else:
            self.lambda_par = None
    ...
    def getLambda_par(self):
        return float(self.lambda_par)
    ...
    def COMPARI(self):
        ...
        lambda_par = self.getLambda_par()
         
        print('The lambda_par is :',lambda_par)
        
        edemaIndex = np.where((iso >lambda_par) & (iso<2.90e-9))
        nonEdemaIndex = np.where(iso <= lambda_par  )
        
        freeWater = np.where(iso >= 2.90e-9)
        ...
        if len(freeWater[0]>0):
            vf_FW[freeWater] = 1
            vf_HW[freeWater] = 0
            vf_stick[freeWater] = 0
            vf_zepp[freeWater] = 0
            partial_volume_0[freeWater] = 1
            partial_volume_1[freeWater] = 0
            AI[freeWater] = 0
            listIndex = list(zip(freeWater[0],freeWater[1],freeWater[2]))
            index =0
            for i in listIndex:
                fod[i[0],i[1],i[2],:] = np.zeros((1, 1, 1, int((self.lmax+1)*(self.lmax+2)/2) ), dtype=np.float)
                index +=1
            
        ############################################################
        
        return partial_volume_0,partial_volume_1,vf_FW,vf_HW,vf_stick,vf_zepp,fods,AI,aff 
        #return partial_volume_0,partial_volume_1,vf_FW,vf_HW,vf_stick,vf_zepp,aff 
...
def argumentParser():
    ...
    p.add_argument('-p', '--pickle', action='store', metavar='pickle', dest='pickle',
...
def main():
    ...
    #partial_volume_0,partial_volume_1,vf_FW,vf_HW,vf_stick,vf_zepp,aff = info.COMPARI()
